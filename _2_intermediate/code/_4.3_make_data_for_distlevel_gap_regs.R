...
dt[, `:=`(xlo = litomg_18 * n_litomg_18, xim = immg_18 * n_immg_18)]
dt[, bch10:=NULL]
...
dt[, `:=`(lito = lito/nlito, im = im/nim)]
summary(dt[, im])
summary(dt[, lito])
dt[, 'nlito':=NULL]
dt <- dt[major_religion != 'Other' & major_religion != 'No Religion']
c <- dt[major_religion == 'Christian', c('iso', 'district', 'lito', 'im', 'nim')]
m <- dt[major_religion == 'Muslim', c('iso', 'district', 'lito', 'im', 'nim')]
t <- dt[major_religion == 'Traditional', c('iso', 'district', 'lito',  'im', 'nim')]
names(c) <- c('iso', 'district', 'litoc', 'imc', 'nimc')
names(m) <- c('iso', 'district', 'litom', 'imm', 'nimm')
names(t) <- c('iso', 'district', 'litot',  'imt', 'nimt')
dt <- merge(c, m, by = c('iso', 'district'), all=TRUE)
dt <- merge(dt, t, by = c('iso', 'district'), all=TRUE)
...
fwrite(dt, '_data_for_distlevel_gapregs.csv')
