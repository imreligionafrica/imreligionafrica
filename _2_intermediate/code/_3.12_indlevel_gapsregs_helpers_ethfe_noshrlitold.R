...
prepare_formula <- function(specification, updn, package) {
  ...
  if (specification==1) {
    ...
    string_formula <- sprintf('%s ~ muslim + traditional | cbd + age + eth_hrm', updn)
  } else if (specification==3) {
    string_formula <- sprintf('%s ~ muslim + traditional%s | cbd + age + eth_hrm', updn, block2)
  } else if (specification==4) {
    string_formula <- sprintf('%s ~ muslim + traditional%s%s | cbd + age + eth_hrm', updn, block2, block3)
  } else if (specification %in% c(5, 6, 7)) {
    string_formula <- sprintf('%s ~ muslim + traditional%s%s | cbd + age + dur + eth_hrm', updn, block2, block3)
  }
  ...
  if (package=='fixest') {
    ...
    if (specification==1) {
      formula <- as.formula(paste0(string_formula, '| eth_hrm | 0 | cbd'))
    } else {
