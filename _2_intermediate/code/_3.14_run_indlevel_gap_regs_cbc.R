...
for (iiso in 1:21) {
  ...
  for (spl in sampvec) {
    ...
    for (updn in dirvec) {
      ...
      #####################################################
      print(sprintf('%s, %s, + religion-district-specific share of literate old', spl, updn))
      if (isoc != 'EGY' & isoc != 'SEN' & isoc != 'MUS') {
        f <- sprintf('%s ~ muslim + traditional | bd + age | 0 | pbd', updn)
      } else {
        f <- sprintf('%s ~ muslim | bd + age | 0 | pbd', updn)
      }
      m <- felm(as.formula(f), data=dtestud)
      outm[controls==contvec[1] & sample==spl & direction==updn,
           `:=`(b=m$coefficients[1],
      ...
      outm[controls==contvec[1] & sample==spl & direction==updn,
           N:=m$N]
      if (isoc != 'EGY' & isoc != 'SEN' & isoc != 'MUS') {
        outt[controls==contvec[1] & sample==spl & direction==updn,
             `:=`(b=m$coefficients[2],
      ...
      m <- felm(as.formula(f), data=dtestud)
      outm[controls==contvec[2] & sample==spl & direction==updn,
           `:=`(b=m$coefficients[1],
                se=sqrt(m$clustervcv[1,1])
           )
      ]
      outm[controls==contvec[2] & sample==spl & direction==updn,
           N:=m$N]
      if (isoc != 'EGY' & isoc != 'SEN' & isoc != 'MUS') {
        outt[controls==contvec[2] & sample==spl & direction==updn,
             `:=`(b=m$coefficients[2],
                  se=sqrt(m$clustervcv[2,2])
             )
        ]
      ...
      m <- felm(as.formula(f), data=dtestud)
      outm[controls==contvec[3] & sample==spl & direction==updn,
           `:=`(b=m$coefficients[1],
                se=sqrt(m$clustervcv[1,1])
           )
      ]
      outm[controls==contvec[3] & sample==spl & direction==updn,
           N:=m$N]
      if (isoc != 'EGY' & isoc != 'SEN' & isoc != 'MUS') {
        outt[controls==contvec[3] & sample==spl & direction==updn,
             `:=`(b=m$coefficients[2],
                  se=sqrt(m$clustervcv[2,2])
             )
        ]
      ...
      if (isoc != 'EGY' & isoc != 'SEN' & isoc != 'MUS') {
        f <- sprintf('%s ~ muslim + traditional%s%s%s | bd + age | 0 | pbd', updn, block1, block2, block3)
      } else {
        f <- sprintf('%s ~ muslim%s%s%s | bd + age | 0 | pbd', updn, block1, block2, block3)
      }
      m <- felm(as.formula(f), data=dtestud)
      outm[controls==contvec[4] & sample==spl & direction==updn,
           `:=`(b=m$coefficients[1],
                se=sqrt(m$clustervcv[1,1])
           )
      ]
      outm[controls==contvec[4] & sample==spl & direction==updn,
           N:=m$N]
      if (isoc != 'EGY' & isoc != 'SEN' & isoc != 'MUS') {
        outt[controls==contvec[4] & sample==spl & direction==updn,
             `:=`(b=m$coefficients[2],
                  se=sqrt(m$clustervcv[2,2])
             )
        ]
      ...
      if (isoc != 'EGY' & isoc != 'SEN' & isoc != 'MUS') {
        f <- sprintf('%s ~ muslim + traditional%s%s%s | bd + age + dur | 0 | pbd', updn, block1, block2, block3)
      } else {
        f <- sprintf('%s ~ muslim%s%s%s | bd + age + dur | 0 | pbd', updn, block1, block2, block3)
      }
      m <- felm(as.formula(f), data=dtestud)
      outm[controls==contvec[5] & sample==spl & direction==updn,
           `:=`(b=m$coefficients[1],
                se=sqrt(m$clustervcv[1,1])
           )
      ]
      outm[controls==contvec[5] & sample==spl & direction==updn,
           N:=m$N]
      if (isoc != 'EGY' & isoc != 'SEN' & isoc != 'MUS') {
        outt[controls==contvec[5] & sample==spl & direction==updn,
             `:=`(b=m$coefficients[2],
                  se=sqrt(m$clustervcv[2,2])
             )
        ]
      ...
      m <- felm(as.formula(f), data=dtestud[similar_m==1])
      outm[controls==contvec[6] & sample==spl & direction==updn,
           `:=`(b=m$coefficients[1],
                se=sqrt(m$clustervcv[1,1])
           )
      ]
      outm[controls==contvec[6] & sample==spl & direction==updn,
           N:=m$N]
      if (isoc != 'EGY' & isoc != 'SEN' & isoc != 'MUS') {
        outt[controls==contvec[6] & sample==spl & direction==updn,
             `:=`(b=m$coefficients[2],
                  se=sqrt(m$clustervcv[2,2])
             )
        ]
      ...
      if (isoc != 'EGY' & isoc != 'SEN' & isoc != 'MUS') {
        ...
        m <- felm(as.formula(f), data=dtestud[similar_t==1])
        outm[controls==contvec[7] & sample==spl & direction==updn,
             `:=`(b=m$coefficients[1],
                  se=sqrt(m$clustervcv[1,1])
             )
        ]
        outm[controls==contvec[7] & sample==spl & direction==updn,
             N:=m$N]
        if (isoc != 'EGY' & isoc != 'SEN' & isoc != 'MUS') {
          outt[controls==contvec[7] & sample==spl & direction==updn,
               `:=`(b=m$coefficients[2],
                    se=sqrt(m$clustervcv[2,2])
               )
          ]
...
OUT <- rbindlist(OUT)
fwrite(OUT, '_indlevel_gapregs_results_cbc.csv')
