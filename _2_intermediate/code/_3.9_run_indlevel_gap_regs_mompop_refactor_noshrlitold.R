...
setwd(codedir)
source("_3.10_indlevel_gapsregs_helpers_mompop_noshrlitold.R") 
...
dt[, ec0 := NULL]
dt[, ec0 := NA]
dt[, ec0 := as.numeric(ec0)]
dt[is.na(ecpop), ec0 := ecmom]
dt[is.na(ecmom), ec0 := ecpop]
dt[!is.na(ecmom) & !is.na(ecpop), ec0 := round((ecmom + ecpop)/2)]
dt <- dt[!is.na(ec0)]
...
fwrite(out, '_indlevel_gapregs_mompop_results_noshrlitold.csv')
