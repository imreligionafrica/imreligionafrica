...
indir <- file.path(rootdir, "_2_intermediate", "code")
...
ISO <- c("BEN", "BFA", "BWA", "ETH", "GHA",
         "LBR", "MLI", "MOZ", "MWI", "SEN",
         "SLE", "TGO", "UGA", "ZAF", "ZMB")
dt <- dt[iso %in% ISO & !is.na(eth_hrm)]
...
for (spl in sampvec) {
  ...
  for (updn in dirvec) {
    ...
    f <- sprintf('%s ~ muslim + traditional | eth_hrm | 0 | cbd', updn)
    ...
    f <- sprintf('%s ~ muslim + traditional | cbd + age + eth_hrm | 0 | cbd', updn)
    m <- felm(as.formula(f), data=dtestud, weights=dtestud[,wt])
    ...
    f <- sprintf('%s ~ muslim + traditional%s | cbd + age + eth_hrm | 0 | cbd', updn, block1)
    m <- felm(as.formula(f), data=dtestud, weights=dtestud[,wt])
    ...
    f <- sprintf('%s ~ muslim + traditional%s%s | cbd + age + eth_hrm | 0 | cbd', updn, block1, block2)
    m <- felm(as.formula(f), data=dtestud, weights=dtestud[,wt])
    ...
    f <- sprintf('%s ~ muslim + traditional%s%s%s | cbd + age + eth_hrm | 0 | cbd', updn, block1, block2, block3)
    m <- felm(as.formula(f), data=dtestud, weights=dtestud[,wt])
    ...
    f <- sprintf('%s ~ muslim + traditional%s%s%s | cbd + age + dur + eth_hrm | 0 | cbd', updn, block1, block2, block3)
    m <- felm(as.formula(f), data=dtestud, weights=dtestud[,wt])
...
fwrite(out, '_indlevel_gapregs_results_ethfe.csv')
